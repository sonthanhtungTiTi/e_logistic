    ...
    <div className="space-y-6 pb-12 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-4">
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Duyệt Đơn Hàng (Order Approval)</h1>
        ...
        <div className="flex items-center gap-2 bg-slate-100 dark:bg-slate-900 p-1 rounded-2xl border border-slate-200 dark:border-slate-800 text-xs font-bold">
          ...
          >
            ...
            {pendingCount > 0 && (
              <span className="px-1.5 py-0.2 rounded-full bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-200 text-[10px] font-mono">
                {pendingCount}
              </span>
            )}
          </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-2xl font-black text-blue-600 dark:text-blue-400 font-mono">{pendingCount} <span className="text-xs font-normal text-slate-400">đơn</span></div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-2xl font-black text-slate-900 dark:text-slate-200 font-mono">{approvedWaitingDispatchCount} <span className="text-xs font-normal text-slate-400">đơn</span></div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-2xl font-black text-blue-600 dark:text-blue-400 font-mono">{assignedDriverCount} <span className="text-xs font-normal text-slate-400">đơn</span></div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-sm flex flex-wrap items-center gap-3">
        <div className="flex-1 min-w-[240px] relative">
          ...
          <input
            ...
            className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl pl-9 pr-3 py-2 text-xs text-slate-800 dark:text-slate-200 focus:border-blue-500 outline-none placeholder:text-slate-400 dark:placeholder:text-slate-600 font-mono"
          />
        </div>
      ...
      {activeTab === 'PENDING' && (
        <div className="space-y-4">
          ...
          {selectedIds.length > 0 && (
            <div className="p-4 bg-blue-50 dark:bg-blue-950/50 border border-blue-200 dark:border-blue-500/40 rounded-2xl flex items-center justify-between shadow-lg animate-fade-in">
              <div className="flex items-center gap-3">
                <span className="w-8 h-8 rounded-xl bg-blue-600 text-white font-black text-xs flex items-center justify-center shadow-sm">
                  {selectedIds.length}
                </span>
                <div>
                  <p className="text-xs font-bold text-slate-900 dark:text-white">
                    Đã chọn <span className="text-blue-600 dark:text-blue-400 font-black">{selectedIds.length}</span> đơn hàng chờ duyệt
                  </p>
                  <p className="text-[10px] text-slate-500 dark:text-slate-400">Phê duyệt đồng loạt để chuyển sang danh sách điều phối gom hàng</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs flex items-center gap-2 shadow-md shadow-blue-600/20 transition cursor-pointer disabled:opacity-50"
                >
                ...
                <button
                  ...
                  className="px-3 py-2 rounded-xl bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 text-xs font-semibold cursor-pointer transition"
                >
                  Bỏ Chọn
                </button>
              </div>
                  ...
                  {loading ? (
                    <tr>
                      <td colSpan={7} className="p-12 text-center text-slate-400">
                        ...
                        <p className="font-semibold text-xs text-slate-600 dark:text-slate-300">Đang tải danh sách đơn chờ duyệt...</p>
                      </td>
                    </tr>
                  ) : orders.length === 0 ? (
                    <tr>
                      <td colSpan={7} className="p-16 text-center text-slate-500 space-y-3">
                        <div className="w-14 h-14 rounded-2xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-blue-600 dark:text-blue-400 mx-auto">
                          <CheckCircle2 className="w-8 h-8" />
                        </div>
                        <div>
                          <p className="font-bold text-sm text-slate-800 dark:text-slate-200">Không có đơn hàng nào đang chờ duyệt</p>
                    ...
                    orders.map((ord) => {
                      ...
                      return (
                        <tr
                          key={ord._id}
                          className={`transition ${isSelected ? 'bg-blue-50/50 dark:bg-blue-950/20 border-l-2 border-l-blue-600' : 'hover:bg-slate-50 dark:hover:bg-slate-800/40'
                            }`}
                        >
                          ...
                          <td className="p-3.5">
                            <div className="flex flex-col gap-1">
                              ...
                              <div className="flex items-center gap-1.5">
                                <span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-blue-50 dark:bg-blue-500/15 text-blue-600 dark:text-blue-300 border border-blue-200 dark:border-blue-500/30">
                                  PENDING_APPROVAL
                                </span>
                          ...
                          <td className="p-3.5">
                            <div className="space-y-0.5 font-mono">
                              <div className="text-blue-600 dark:text-blue-400 font-bold">Cước: {formatCurrency(ord.shippingFee)}</div>
                              <div className="text-slate-800 dark:text-slate-200">COD: {formatCurrency(ord.codAmount)}</div>
                              <div className="text-[11px] text-slate-500 dark:text-slate-400">TL: {ord.chargeableWeight || 0} kg</div>
                            </div>
                          </td>
                          ...
                          <td className="p-3.5 text-right">
                            <div className="flex items-center justify-end gap-2">
                              <button
                                ...
                                className="p-1.5 rounded-lg bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 transition cursor-pointer"
                  ...
                  {loadingApproved ? (
                    <tr>
                      <td colSpan={6} className="p-12 text-center text-slate-400">
                        <RefreshCw className="w-8 h-8 text-blue-600 animate-spin mx-auto mb-2" />
                        <p className="font-semibold text-xs text-slate-600 dark:text-slate-300">Đang tải danh sách đơn đã duyệt...</p>
                      </td>
                    </tr>
                  ) : approvedOrders.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="p-16 text-center text-slate-500 space-y-3">
                        ...
                        <div>
                          <p className="font-bold text-sm text-slate-800 dark:text-slate-200">Chưa có đơn hàng nào đã duyệt</p>
                          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                    ...
                    approvedOrders.map((ord) => {
                      ...
                      return (
                        <tr key={ord._id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition">
                          ...
                          <td className="p-3.5">
                            {isAssigned ? (
                              <span className="px-2 py-1 rounded-full font-bold text-[10px] uppercase border inline-flex items-center gap-1 bg-blue-50 dark:bg-blue-500/20 text-blue-700 dark:text-blue-300 border-blue-200 dark:border-blue-500/30">
                                <Truck className="w-3 h-3" /> ĐÃ GÁN TÀI XẾ GOM
                              </span>
                            ) : (
                              <span className="px-2 py-1 rounded-full font-bold text-[10px] uppercase border inline-flex items-center gap-1 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-700">
                          ...
                          <td className="p-3.5 font-mono">
                            <div className="text-blue-600 dark:text-blue-400 font-bold">{formatCurrency(ord.shippingFee)}</div>
                            <div className="text-slate-800 dark:text-slate-200">COD: {formatCurrency(ord.codAmount)}</div>
                          </td>
      ...
      {selectedOrderDetail && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="w-full max-w-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-2xl space-y-5 text-xs">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="p-3 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 space-y-1">
                <span className="text-[10px] font-bold text-blue-600 dark:text-blue-400 uppercase">Thông tin Shop (Người Gửi)</span>
                <p className="font-bold text-slate-900 dark:text-white text-xs">{selectedOrderDetail.pickupAddress?.fullName}</p>
                <p className="text-slate-500 dark:text-slate-400 font-mono">{selectedOrderDetail.pickupAddress?.phone}</p>
                <p className="text-slate-500 dark:text-slate-400">{selectedOrderDetail.pickupAddress?.address}, {selectedOrderDetail.pickupAddress?.district}, {selectedOrderDetail.pickupAddress?.province}</p>
              </div>

              <div className="p-3 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 space-y-1">
                <span className="text-[10px] font-bold text-blue-600 dark:text-blue-400 uppercase">Thông tin Người Nhận</span>
                <p className="font-bold text-slate-900 dark:text-white text-xs">{selectedOrderDetail.deliveryAddress?.fullName}</p>
                <p className="text-slate-500 dark:text-slate-400 font-mono">{selectedOrderDetail.deliveryAddress?.phone}</p>
                <p className="text-slate-500 dark:text-slate-400">{selectedOrderDetail.deliveryAddress?.address}, {selectedOrderDetail.deliveryAddress?.district}, {selectedOrderDetail.deliveryAddress?.province}</p>
              </div>
            </div>

            {/* Items */}
            <div className="p-3 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 space-y-2">
              <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase">Sản phẩm bưu gửi</span>
              <div className="divide-y divide-slate-200 dark:divide-slate-800">
                {(selectedOrderDetail.items || []).map((it, idx) => (
                  <div key={idx} className="py-1.5 flex justify-between">
                    <span className="text-slate-800 dark:text-slate-200">{it.name} (x{it.quantity})</span>
                    <span className="font-mono text-slate-500 dark:text-slate-400">{it.weight} kg</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Financials */}
            <div className="flex justify-between items-center p-3 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 font-mono">
              <div>
                <span className="text-slate-500 block text-[10px]">CƯỚC PHÍ</span>
                <span className="text-blue-600 dark:text-blue-400 font-bold text-sm">{formatCurrency(selectedOrderDetail.shippingFee)}</span>
              </div>
              <div>
                <span className="text-slate-500 block text-[10px]">TIỀN THU HỘ (COD)</span>
                <span className="text-slate-900 dark:text-white font-bold text-sm">{formatCurrency(selectedOrderDetail.codAmount)}</span>
              </div>
              <div>
                <span className="text-slate-500 block text-[10px]">TRỌNG LƯỢNG</span>
                <span className="text-blue-600 dark:text-blue-400 font-bold text-sm">{selectedOrderDetail.chargeableWeight || 0} kg</span>
              </div>
            </div>
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-slate-200 dark:border-slate-800">
              <button
                onClick={() => setSelectedOrderDetail(null)}
                className="px-4 py-2 rounded-xl bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 font-bold cursor-pointer"
              >
                Đóng
              </button>
              {selectedOrderDetail.status === 'PENDING_APPROVAL' ? (
                <button
                  ...
                  className="px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold flex items-center gap-1 cursor-pointer shadow-md shadow-blue-600/20"
                >
