...
export const DispatchControlPage: React.FC = () => {
  ...
  const handleCommitAutoAssign = async () => {
    ...
    try {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-4">
        <div>
          <div className="flex items-center gap-2">
            <span className="p-2 rounded-xl bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/30 text-blue-600 dark:text-blue-400">
        ...
        {/* Tab switcher */}
        <div className="flex items-center gap-2 bg-white dark:bg-slate-900 p-1 rounded-2xl border border-slate-200 dark:border-slate-800 text-xs font-bold shadow-sm">
          <button
          ...
          >
            ...
            {approvedOrders.length > 0 && (
              <span className="px-1.5 py-0.2 rounded-full bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 text-[10px] font-mono">
      ...
      <div className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-2xl p-3 px-4 flex flex-wrap items-center justify-between gap-3 text-xs shadow-sm">
        <div className="flex items-center gap-2 text-slate-700 dark:text-slate-300">
          <span className="p-1.5 rounded-lg bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/20">
            <Bike className="w-4 h-4" />
          </span>
          <span>
            <strong className="text-emerald-600 dark:text-emerald-400">Chặng Gom Hàng Tận Nơi:</strong> Phân công <strong>Xe máy</strong> luồn lách ngõ hẻm vào tận nơi lấy hàng của Shop (giới hạn an toàn &le; 30kg/đơn).
          </span>
        </div>
        <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400">
          <span className="p-1.5 rounded-lg bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-500/20">
            <Truck className="w-4 h-4" />
          </span>
          <span>
            <strong className="text-blue-600 dark:text-blue-400">Chặng Luân Chuyển Liên Kho:</strong> Dùng <strong>Xe tải</strong> chuyên chở bao hàng giữa các kho/Hub bưu cục (quản lý tại Outbound Trips).
          </span>
        </div>
      </div>
      ...
      {activeTab === 'MASTER_DISPATCH' ? (
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-sm flex flex-wrap items-center justify-between gap-4">
            <div className="flex flex-wrap items-center gap-3 flex-1">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-slate-800 dark:text-white">Tài xế nhận lệnh:</span>
              </div>

              <select
                value={selectedDriverId}
                onChange={(e) => setSelectedDriverId(e.target.value)}
                className="bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-xl px-3 py-2 text-xs text-slate-900 dark:text-white font-bold outline-none focus:border-blue-500 min-w-[320px]"
              >
                {drivers.length === 0 ? (
              ...
              {selectedDriver && (
                <div className="hidden sm:flex items-center gap-2 text-[11px] px-3 py-1.5 rounded-xl bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-500/30 text-blue-800 dark:text-blue-200">
                  <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                  <span>
                    Ưu tiên tài xế xe máy ít đơn nhất: <strong className="text-slate-900 dark:text-white">{selectedDriver.fullName}</strong> ({selectedDriver.activeOrdersCount || 0} đơn đang gom)
                  </span>
                </div>
              )}
              ...
              <button
                ...
                className="px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs flex items-center gap-2 shadow-md shadow-blue-600/20 transition cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
              >
              ...
              <button
                ...
                className="px-4 py-2 rounded-xl bg-blue-700 hover:bg-blue-600 text-white font-bold text-xs flex items-center gap-2 shadow-md shadow-blue-700/20 transition cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={fetchData}
                className="p-2 rounded-xl bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 transition cursor-pointer shadow-sm"
          ...
          {/* Filter Bar */}
          <div className="bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-sm flex flex-wrap items-center gap-3">
            <div className="flex-1 min-w-[240px] relative">
              ...
              <input
                ...
                className="w-full bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl pl-9 pr-3 py-2 text-xs text-slate-900 dark:text-slate-200 focus:border-blue-500 outline-none placeholder:text-slate-400 font-mono"
              />
            </div>
            ...
            <div className="w-48">
              <input
                ...
                className="w-full bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2 text-xs text-slate-900 dark:text-slate-200 focus:border-blue-500 outline-none placeholder:text-slate-400"
              />
            </div>

            <div className="text-xs text-slate-600 dark:text-slate-400 font-bold px-2">
              Đơn đã duyệt chờ phân tài xế: <span className="text-blue-600 dark:text-blue-400 font-mono text-sm">{approvedOrders.length}</span>
            </div>
          </div>
                  ...
                  <tr className="bg-slate-50 dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 text-[10px] font-black uppercase text-slate-500 dark:text-slate-400 tracking-wider">
                    <th className="p-3.5 w-10 text-center">
                      <input
                        ...
                        className="rounded border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 text-blue-600 focus:ring-blue-500 cursor-pointer"
                      />
                    </th>
                  ...
                  {loading ? (
                    <tr>
                      <td colSpan={6} className="p-12 text-center text-slate-500 dark:text-slate-400">
                        ...
                        <p className="font-semibold text-xs text-slate-700 dark:text-slate-300">Đang tải danh sách đơn đã duyệt...</p>
                      </td>
                    </tr>
                  ) : approvedOrders.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="p-16 text-center text-slate-500 space-y-3">
                        <div className="w-14 h-14 rounded-2xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-emerald-600 dark:text-emerald-400 mx-auto">
                          <CheckCircle2 className="w-8 h-8" />
                        </div>
                        <div>
                          <p className="font-bold text-sm text-slate-800 dark:text-slate-200">Không có đơn hàng nào chờ phân tài xế gom</p>
                      ...
                      return (
                        <tr
                          ...
                          className={`transition ${
                            isSelected ? 'bg-blue-50/70 dark:bg-blue-950/20 border-l-2 border-l-blue-600' : 'hover:bg-slate-50 dark:hover:bg-slate-800/40'
                          }`}
                        >
                          ...
                          <td className="p-3.5">
                            <div className="flex flex-col gap-1">
                              <span className="font-mono font-bold text-blue-600 dark:text-blue-400 text-sm">{ord.trackingCode}</span>
                              <div className="flex items-center gap-1.5">
                                <span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-blue-50 dark:bg-blue-500/20 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-500/30">
                                  APPROVED
                                </span>
                          ...
                          <td className="p-3.5">
                            <div className="space-y-0.5">
                              <div className="text-slate-800 dark:text-slate-200 font-bold">{ord.deliveryAddress?.fullName}</div>
                              <div className="text-[11px] text-slate-500 dark:text-slate-400 font-mono">{ord.deliveryAddress?.phone}</div>
                          ...
                          <td className="p-3.5">
                            <div className="space-y-0.5 font-mono">
                              <div className="text-slate-800 dark:text-slate-200">TL: {ord.chargeableWeight || 0} kg</div>
                          ...
                          <td className="p-3.5 text-right">
                            <button
                              ...
                              className="px-3 py-1.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs inline-flex items-center gap-1.5 shadow-md shadow-blue-600/20 transition cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
                            >
                              <Send className="w-3.5 h-3.5" />
                              <span>
      ...
      {isPreviewOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-xs animate-fade-in">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-blue-500/40 rounded-3xl w-full max-w-5xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
            {/* Modal Header */}
            <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <span className="p-2.5 rounded-2xl bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/30 text-blue-600 dark:text-blue-400">
                  <Zap className="w-6 h-6 fill-blue-600 dark:fill-blue-400 text-blue-600 dark:text-blue-400" />
                </span>
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="px-2 py-0.5 rounded-full bg-blue-50 dark:bg-blue-500/20 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-500/30 text-[10px] font-bold">
            ...
            <div className="p-6 flex-1 overflow-y-auto space-y-6">
              {loadingPreview ? (
                <div className="py-20 text-center space-y-3">
                  ...
                  <p className="font-bold text-sm text-slate-800 dark:text-slate-200">Đang tính toán phân bổ tối ưu...</p>
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    <div className="bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 rounded-2xl p-3">
                      <span className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 block">Tổng đơn xét duyệt</span>
                      <span className="text-xl font-black text-slate-900 dark:text-white font-mono">{previewData.totalOrders}</span>
                    </div>

                    <div className="bg-emerald-50 dark:bg-slate-950/60 border border-emerald-200 dark:border-emerald-500/30 rounded-2xl p-3">
                      <span className="text-[11px] font-semibold text-emerald-800 dark:text-slate-400 block">Đã có phương án</span>
                      <span className="text-xl font-black text-emerald-600 dark:text-emerald-400 font-mono">{previewData.assignedCount} đơn</span>
                    </div>

                    <div className={`border rounded-2xl p-3 ${
                      previewData.unassignedCount > 0 ? 'bg-rose-50 dark:bg-slate-950/60 border-rose-200 dark:border-rose-500/40' : 'bg-slate-50 dark:bg-slate-950/60 border-slate-200 dark:border-slate-800'
                    }`}>
                      <span className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 block">Chưa thể gán</span>
                      <span className={`text-xl font-black font-mono ${
                        previewData.unassignedCount > 0 ? 'text-rose-600 dark:text-rose-400' : 'text-slate-700 dark:text-slate-400'
                      }`}>
                        {previewData.unassignedCount} đơn
                      </span>
                    </div>

                    <div className="bg-blue-50 dark:bg-slate-950/60 border border-blue-200 dark:border-blue-500/30 rounded-2xl p-3">
                      <span className="text-[11px] font-semibold text-blue-800 dark:text-slate-400 block">Tài xế xe máy tham gia</span>
                  ...
                  {previewData.driversSummary && previewData.driversSummary.length > 0 && (
                    <div className="p-3 bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-500/30 rounded-2xl space-y-2">
                        ...
                        {previewData.driversSummary.map((d) => (
                          <div
                            ...
                            className={`px-2.5 py-1 rounded-xl text-[11px] font-semibold border flex items-center gap-1.5 ${
                              d.newAssignedInBatch > 0
                                ? 'bg-white dark:bg-blue-900/40 border-blue-300 dark:border-blue-500/40 text-blue-900 dark:text-white'
                          ...
                          >
                            ...
                            {d.newAssignedInBatch > 0 ? (
                              ...
                              <span className="font-mono text-slate-400 dark:text-slate-500">0 đơn</span>
                            )}
                          </div>
                        ))}
                      </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="bg-white dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm">
                      <div className="overflow-x-auto max-h-[360px]">
                        <table className="w-full text-left text-xs border-collapse">
                          <thead className="sticky top-0 bg-slate-50 dark:bg-slate-950 z-10 border-b border-slate-200 dark:border-slate-800 text-[10px] font-black uppercase text-slate-500 dark:text-slate-400">
                            <tr>
                          ...
                          <tbody className="divide-y divide-slate-200 dark:divide-slate-800/60">
                            {previewData.assignments.map((item) => (
                              <tr key={item.orderId} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition">
                                <td className="p-3">
                                  <div className="flex flex-col gap-0.5">
                                    <span className="font-mono font-bold text-blue-600 dark:text-blue-400">{item.trackingCode}</span>
                                ...
                                <td className="p-3">
                                  <select
                                    ...
                                    className="bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl px-2.5 py-1.5 text-xs text-slate-900 dark:text-white font-bold outline-none focus:border-blue-500 min-w-[200px]"
                                  >
                                    {drivers.map((d) => (
                                ...
                                <td className="p-3">
                                  <div className="space-y-0.5">
                                    <span className="px-1.5 py-0.5 rounded bg-emerald-50 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-500/30 text-[10px] font-bold font-mono">
                                      Score: {item.score}
                                    </span>
                                    <div className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">{item.reason}</div>
                                  </div>
                                </td>
                              </tr>
                            ))}
                  ...
                  {previewData.unassigned && previewData.unassigned.length > 0 && (
                    <div className="p-4 rounded-2xl bg-rose-50 dark:bg-rose-950/20 border border-rose-200 dark:border-rose-500/30 space-y-2">
                      <div className="flex items-center gap-2 text-xs font-bold text-rose-600 dark:text-rose-400">
                        <AlertCircle className="w-4 h-4" />
                        <span>Các đơn chưa thể gán tự động ({previewData.unassigned.length} đơn):</span>
                      </div>
                      <div className="divide-y divide-rose-200 dark:divide-rose-950/40 text-xs">
                        {previewData.unassigned.map((u) => (
                          <div key={u.orderId} className="py-2 flex items-center justify-between gap-4">
                            <span className="font-mono font-bold text-slate-800 dark:text-slate-200">{u.trackingCode}</span>
                            <span className="text-slate-500 dark:text-slate-400">
                              {u.pickupAddress?.district}, {u.pickupAddress?.province}
                            </span>
                            <span className="text-rose-600 dark:text-rose-300 font-semibold">{u.reason}</span>
                          </div>
                        ))}
                      </div>
                    </div>
            ...
            <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 flex flex-col sm:flex-row items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="px-5 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs flex items-center gap-2 shadow-md shadow-blue-600/20 transition cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
                >
