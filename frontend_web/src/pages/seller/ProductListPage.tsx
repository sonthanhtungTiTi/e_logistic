...
export const ProductListPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      {toastMessage && (
        <div
          className={`p-4 rounded-2xl text-xs font-bold flex items-center justify-between shadow-lg gap-2 ${
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800/80 pb-4 sm:pb-5">
        <div>
          <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400 mb-1">
            ...
            <span className="text-indigo-600 dark:text-indigo-400 font-semibold">Danh Mục Sản Phẩm Mẫu</span>
          </div>
          <h2 className="text-2xl font-black text-slate-900 dark:text-white flex items-center gap-2.5">
            <Boxes className="w-7 h-7 text-indigo-600 dark:text-indigo-400 shrink-0" /> Quản Lý Sản Phẩm Mẫu (Catalog)
          </h2>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            ...
            className="px-4 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs flex items-center gap-2 shadow-md shadow-blue-600/20 transition cursor-pointer"
          >
      ...
      <div className="glass-panel rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm">
        {loading ? (
          ...
          <div className="p-12 text-center text-slate-500 dark:text-slate-400">
            <Boxes className="w-12 h-12 text-slate-400 dark:text-slate-600 mx-auto mb-3" />
            <p className="text-sm font-bold text-slate-900 dark:text-white mb-1">Chưa có sản phẩm mẫu nào</p>
                  ...
                  return (
                    <tr key={p._id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition">
                      <td className="p-3.5 font-mono font-bold text-indigo-600 dark:text-indigo-400">
                        {p.sku || <span className="text-slate-400 dark:text-slate-600 italic">Chưa đặt SKU</span>}
                      </td>
                      <td className="p-3.5">
                        <div className="font-bold text-slate-900 dark:text-white text-sm">{p.name}</div>
                        {p.description && (
                          <div className="text-[11px] text-slate-500 dark:text-slate-400 truncate max-w-xs">{p.description}</div>
                        )}
                      </td>
                      <td className="p-3.5">
                        <span className="px-2.5 py-0.5 rounded-md bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 font-semibold text-[11px]">
                          {p.category}
                        </span>
                      </td>
                      <td className="p-3.5">
                        <div className="flex items-center gap-1 font-mono font-bold text-emerald-600 dark:text-emerald-400">
                          <Scale className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-500" />
                          {p.weightKg} kg
                        </div>
                      </td>
                      <td className="p-3.5">
                      ...
                      <td className="p-3.5 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button
                            ...
                            className="p-1.5 rounded-lg bg-rose-50 hover:bg-rose-100 dark:bg-rose-950/40 dark:hover:bg-rose-900/60 text-rose-600 dark:text-rose-300 border border-rose-200 dark:border-rose-800/40 transition cursor-pointer"
                            title="Xóa sản phẩm"
                          >
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center px-6 py-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-950/60">
              ...
              <button
                ...
                className="text-slate-400 hover:text-slate-700 dark:hover:text-white transition p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 text-xs">
              <div>
                ...
                <input
                  ...
                  className="w-full glass-input rounded-xl px-3.5 py-2.5 text-slate-900 dark:text-white bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700"
                />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-700 dark:text-slate-300 font-bold mb-1">Mã SKU (Tùy chọn)</label>
                  <input
                    type="text"
                    placeholder="VD: AT-COT-01"
                    value={sku}
                    onChange={(e) => setSku(e.target.value)}
                    className="w-full glass-input rounded-xl px-3.5 py-2.5 text-slate-900 dark:text-white bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 font-mono uppercase"
                  />
                </div>
                <div>
                  <label className="block text-slate-700 dark:text-slate-300 font-bold mb-1">Danh mục</label>
                  <select
                    value={category}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full glass-input rounded-xl px-3.5 py-2.5 text-slate-900 dark:text-white bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 font-mono"
                  />
                </div>
                <div>
                  <label className="block text-slate-700 dark:text-slate-300 font-bold mb-1">Giá trị hàng (VNĐ)</label>
                  <input
                    type="number"
                    step="1000"
                    min="0"
              ...
              <div>
                <label className="block text-slate-700 dark:text-slate-300 font-bold mb-1">Kích thước bưu kiện (Dài x Rộng x Cao cm)</label>
                <div className="grid grid-cols-3 gap-2">
                  <div className="relative">
                    <input
                      ...
                      className="w-full glass-input rounded-xl px-3 py-2 text-slate-900 dark:text-white bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 font-mono text-center"
                    />
                    <span className="text-[10px] text-slate-400 absolute right-2 top-2">cm</span>
                  </div>
                  <div className="relative">
                    <input
                      type="number"
                ...
                <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-1">
                  ...
                  <b className="text-amber-600 dark:text-amber-400 font-mono">
              ...
              <div>
                <label className="block text-slate-700 dark:text-slate-300 font-bold mb-1">Mô tả sản phẩm (Tùy chọn)</label>
                <textarea
                  rows={2}
                  placeholder="Ghi chú thêm về chất liệu, đóng gói đặc biệt..."
                  value={description}
                  onChange={(e) => setDescription(e.target.value)}
                  className="w-full glass-input rounded-xl px-3.5 py-2 text-slate-900 dark:text-white bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700"
                />
              </div>
              ...
              <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-200 dark:border-slate-800">
                ...
                <button
                  ...
                  className="px-5 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold flex items-center gap-1.5 shadow-md shadow-indigo-600/20 transition cursor-pointer"
                >
